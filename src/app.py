...
from src.models import Product  # <-- Guarde este nome: Product
...
# CORRIGIDO: Colchete fechado corretamente no final da lista
origins_permitidas = [
    "http://localhost:4200",
...
def get_current_user():
    ...
    if not user:
        raise HTTPError(401, message='Não logado')
...
def find_all_products(filter: dict):
    ...
    query = '''
        SELECT id, name, description, quantity, price, image_path, categoria   
        FROM products
        WHERE 1 = 1
    '''
    ...
    # CORRIGIDO: Alterado de Produto(*row) para Product(*row) conforme o import original
...
def get_product(id):
    ...
    if not row:
        raise HTTPError(404, message='Produto não encontrado')
...
def create_products(product_in: dict):
    ...
    cursor.execute(
        '''
        INSERT INTO products (name, description, quantity, price, image_path, categoria)
        VALUES (?, ?, ?, ?, ?, ?)
        ''',
        parameters
    )
...
# MAIN
# =========================================
# Se precisar forçar a criação automática das tabelas iniciais, descomente as duas linhas abaixo:
